...
export default function AddHoardingBillingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await apiService.post(endPointApi.hoardingBilling, {
        ...formData,
      ...
      console.error('Error saving hoarding bill:', error);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-[#1B2642]">New Hoarding Invoice</h2>
          <p className="text-sm text-gray-500 mt-1">Enter customer and item details to generate a tax invoice</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end pt-2">
          ...
          <button type="submit" disabled={submitting} className="px-8 py-3 bg-[#1B2642] text-white rounded-xl hover:bg-[#1B2642]/90 flex items-center gap-2 font-medium disabled:opacity-50">
            {submitting ? 'Saving...' : <><Plus className="w-5 h-5" /> Save Invoice</>}
          </button>
        </div>
      </form>
    </div>
