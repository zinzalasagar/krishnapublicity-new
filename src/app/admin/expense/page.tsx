...
export default function ExpensePage() {
  ...
  const handleDownloadReport = () => {
    let url = `${endPointApi.serverUrl}/api/${endPointApi.expenseReportPdf}`;
    if (dateValue.startDate && dateValue.endDate) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/admin/expense/add" className="px-4 py-1.5 text-sm bg-[#1B2642] text-white rounded-lg hover:bg-[#1B2642]/90 flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
        <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col justify-center">
          <p className="text-gray-500 text-xs font-medium mb-0.5">કુલ જાવક (Total Expense)</p>
        ...
        <div className="bg-white p-4 rounded-xl shadow-sm border border-green-100 flex flex-col justify-center">
          <p className="text-gray-500 text-xs font-medium mb-0.5">આપેલ રકમ (Amount Paid)</p>
                ...
                entries.map(entry => (
                  <tr key={entry._id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-6 py-2.5 flex items-center gap-2">
                      <a href={`${endPointApi.serverUrl}/api/${endPointApi.expenseBillPdf(entry._id)}`} target="_blank" rel="noreferrer" className="text-green-600 hover:text-green-800 p-2 hover:bg-green-50 rounded-lg transition-colors" title="Download Voucher">
