...
export default function EditExpensePage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchExpense = async () => {
      try {
        const data = await apiService.get(`${endPointApi.expense}/${params.id}`);
        if (data) {
          setFormData({
            details: data.details || '',
            printingPress: data.printingPress || '',
            date: data.date || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await apiService.put(`${endPointApi.expense}/${params.id}`, {
        ...formData,
      ...
      toast.success('જાવક સફળતાપૂર્વક અપડેટ થઈ (Expense updated successfully)!');
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-2xl font-bold text-[#1B2642]">જાવક અપડેટ કરો (Edit Expense)</h2>
          <p className="text-sm text-gray-500 mt-1">જાવકની વિગતો બદલો</p>
        </div>
      </div>
