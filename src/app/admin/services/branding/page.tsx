...
export default function AdminBranding() {
  ...
  const [form, setForm] = useState<BrandingItem>({
    slug: '',
    name: '',
    image: '',
    galleryImages: [],
  ...
  const fetchBranding = async () => {
    try {
      ...
      toast.error('Failed to load branding services');
    } finally {
      setLoading(false);
    }
  };

  const handleOpenModal = (item?: BrandingItem) => {
    if (item) {
      setEditingItem(item);
      setForm({
        slug: item.slug,
  ...
  const handleSave = async () => {
    ...
    const toastId = toast.loading(editingItem ? 'Updating branding service...' : 'Creating branding service...');
    const slug = form.slug || form.name.toLowerCase().replace(/[^a-z0-9]/g, '-');

    try {
      if (editingItem?._id) {
        await apiService.put(`${endPointApi.branding}/${editingItem._id}`, {
          ...form,
          slug
        });
      } else {
        await apiService.post(endPointApi.branding, {
          ...form,
          slug
        });
      }
      toast.success('Branding service saved!', { id: toastId });
      setIsModalOpen(false);
      fetchBranding();
    } catch (error: any) {
      toast.error(error.message || 'Failed to save branding service', { id: toastId });
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    const toastId = toast.loading('Deleting branding service...');
    try {
      await apiService.delete(`${endPointApi.branding}/${deletingId}`);
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-[#1B2642]">Branding Services</h1>
          <p className="text-xs text-gray-500 font-medium mt-1">Manage all brand identity options (Tricycle Ad, Rickshaw Ad, Canopy, etc.)</p>
        </div>
        <button
        ...
        >
          <span>+ Add Branding Service</span>
        </button>
      </div>
        ...
        {items.map((item) => (
          <div key={item._id || item.slug} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100/50 flex flex-col justify-between space-y-4">
            <div className="space-y-3">
              <img
                src={apiService.getImageUrl(item.image, '/branndingImage/t1.jpg')}
                alt={item.name}
      ...
      {/* Branding Service Modal */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingItem ? 'Edit Branding Service' : 'Add Branding Service'}>
        <div className="space-y-4 pt-2">
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Tricycle Ad, Rickshaw Ad, Canopy"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Describe this branding service..."
            />
          </div>
      ...
      <ConfirmModal
        ...
        title="Delete Branding Service"
        message="Are you sure you want to delete this branding service? This action cannot be undone."
      />
    </div>
  );
}
