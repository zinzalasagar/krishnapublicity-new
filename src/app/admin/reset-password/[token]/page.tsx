...
export default function ResetPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiService.put(endPointApi.authResetPassword(token as string), { password });
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F9F7F2] font-sans">
      <div className="max-w-md w-full bg-white px-10 py-12 rounded-[2rem] shadow-[0_8px_40px_rgba(27,38,66,0.06)] border border-[#1B2642]/5 relative">
        <div className="flex flex-col items-center mb-10 text-center">
          ...
          <h1 className="font-serif text-2xl font-semibold text-[#1B2642] mb-1">Reset Password</h1>
          <p className="text-[10px] uppercase tracking-widest text-[#1B2642]/40 font-bold">Enter your new passcode</p>
        </div>
        
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-[#1B2642]/40 text-[10px] font-bold uppercase tracking-wider mb-2" htmlFor="password">
              New Passcode
            </label>
          ...
          <div>
            <label className="block text-[#1B2642]/40 text-[10px] font-bold uppercase tracking-wider mb-2" htmlFor="confirmPassword">
          ...
          <div className="pt-2">
            ...
            >
              {loading ? 'Resetting...' : 'Reset Password'} <span>&rarr;</span>
            </button>
          </div>
        </form>
      </div>
